...
class MAMChannel {
  ...
  /**
   * Change the provider to use from now on
   * @param {String} key - The new key
   */
  changeProvider(newProvider) {
    MAM.provider = newProvider;
  }
  ...
  create(trytes) {
    return MAM.create(this.mamState, trytes);
  }

  attach(message, depth, difficulty) {
    return MAM.attach(message.payload, message.address, depth, difficulty);
  }

  setState(newState) {
    this.mamState = newState;
  }
